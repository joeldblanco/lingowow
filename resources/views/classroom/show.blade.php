    ...
    <x-app-layout>
        <div class="font-bold">
            <div class="py-20 flex flex-col items-center">
                <p class="text-red-500 text-3xl">{{ $message1 }}</span></p>
                <p class="text-gray-700 text-lg">{{ $message2 }}<span id="time-remaining"></p>
            </div>
        </div>
        <script src="https://cdnjs.cloudflare.com/ajax/libs/moment.js/2.29.1/moment.min.js"></script>
        <script>
            var timeRemainingElement = document.getElementById('time-remaining');
            var timeRemaining = @json($time);
            ...
            var intervalId = setInterval(function() {
                ...
                if (timeRemaining <= 0) {
                    ...
                    let dia = (duration.days() > 1 ? duration.days() + " days, " : (duration.days() < 1 ? "" : duration
                        .days() + " day, "))
                    let hora = (duration.hours() > 1 ? duration.hours() + " hours, " : (duration.hours() < 1 ? "" :
                        duration.hours() + " hour, "))
                    let minuto = (duration.minutes() > 1 ? duration.minutes() + " minutes, " : (duration.minutes() < 1 ?
                        "" : duration.minutes() + " minute, "))
                    let segundo = (duration.seconds() > 1 ? duration.seconds() + " seconds " : (duration.seconds() < 1 ?
