<x-app-layout>
    ...
    <a href="{{route('cart')}}" class="bg-green-500 rounded-full w-20 h-20 flex justify-center items-center fixed bottom-5 right-5 z-10">
        @if(Cart::count() > 0)
            <div class="absolute top-0 right-0 rounded-full bg-red-600 w-7 h-7 text-white font-bold flex justify-center">
                {{Cart::count()}}
            </div>
        @endif
