<x-app-layout>
    ...
    @php
        ...
        $user = App\Models\User::find($notification->data[0]);
        $notification->data[1] = str_replace("on ","",$notification->data[1]);

        switch ($notification->type) {
                case "BookedClass":
                    $notification_icon = "fas fa-bookmark";
                    $notification->data = "The student ".$user->first_name." ".$user->last_name." has booked a class ".$notification->data[1];
                break;

                case "ClassRescheduled":
                    $notification_icon = "fas fa-calendar-alt";
                    $notification->data = "The student ".$user->first_name." ".$user->last_name." has rescheduled a class. New schedule: ".$notification->data[1];
                break;

                case "ClassCanceledToStudent":
                    $notification_icon = "fas fa-calendar-alt";
                    $notification->data = "The student ".$user->first_name." ".$user->last_name." has canceled a class. Canceled class: ".$notification->data[1];
                break;
        ...
        $friends = DB::table('friend_requests')->where('sender_id',$user->id)->orWhere('receiver_id',$user->id)->get();
            ...
            <div class="overflow-hidden">
                ...
                <table class="flex m-auto w-3/4 justify-center border-collapse table-auto whitespace-no-wrap table-striped relative h-full">
                    <tbody class="flex flex-col bg-white p-10 rounded-xl w-full">
                        ...
                        @if ($notification->type == ("BookedClass"||"ClassRescheduled"))
                        <tr class="text-gray-700 font-bold space-x-3 flex justify-around items-center pt-5 border-t-2">
                            <td class="text-center border-gray-200 w-full">
                                <div class="w-full rounded-2xl border border-gray-200 p-4 flex flex-row shadow-xl">
                                    <a href="{{route('profile.show',$user->id)}}" class="flex flex-row space-x-3 rounded-2xl w-5/6">
                                        <div class="w-1/6 rounded-2xl"
                                            style="background-image: url('{{Storage::url($user->profile_photo_path)}}'); background-size: cover;">
                                            <img src="{{Storage::url($user->profile_photo_path)}}" alt="pp" class="rounded-2xl">
                                        </div>
                                        <div class="w-4/6 flex flex-col items-start justify-between p-3">
                                            ...
                                            <div class="flex space-x-3 items-center">
                                                ...
                                                <p>{{count($friends)}} @if(count($friends) == 1) Friend @else Friends @endif</p>
                                            </div>
                                        </div>
                                    </a>
                                    <div class="w-1/6 flex flex-col justify-end">
                                        <div class="flex flex-col justify-center rounded-full h-12 border border-gray-500 cursor-pointer hover:bg-gray-900 hover:text-gray-300 transition ease-in duration-200">
                                            <p>Add Friend</p>
                                        </div>
                                    </div>
                                </div>
