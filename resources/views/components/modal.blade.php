@props(['type', 'title', 'title', 'footer', 'open' => 'false', 'name' => 'modal'])

<div>
    <div class="fixed inset-0 w-full h-full z-20 bg-black bg-opacity-50 duration-300 overflow-y-auto" x-show="{{$name}}"
        ...
        <div class="relative sm:w-3/4 md:w-1/2 lg:w-1/3 mx-2 sm:mx-auto my-10 opacity-100">
            <div class="relative bg-white shadow-lg rounded-md text-gray-900 z-20" @click.outside="{{$name}} = false"
                ...
                {{-- MODAL HEADER --}}
                <header {{ $footer->attributes->merge(['class' => 'flex items-center justify-between p-2']) }}>
                ...
                {{-- MODAL CONTENT --}}
                <main {{ $footer->attributes->merge(['class' => 'p-2 text-center']) }}>
                    <p class="my-5">
                        {{ $content }}
                    </p>
                </main>
