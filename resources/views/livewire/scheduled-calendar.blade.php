    ...
    <div class="container mx-auto mt-10" x-data="{ editBtn: true, edit: false, showModal1: false, showModal2: false, showModal3: false, event: {{ $event }}, loadingState: false }" x-cloak>
        <div class="wrapper bg-white rounded shadow w-full">
            ...
            <div class="w-full flex flex-row">
                        ...
                        @else
                            ...
                            <div class="selectable w-full" id="selectable">
                                            ...
                                            @if (in_array([$i + 6, $e], $students_schedules))
                                                @foreach ($students as $student)
                                                    ...
                                                    @if (in_array([$i + 6, $e], $student[1]))
                                                        <div class="w-32 h-10 border cell schedule_cell ui-selected flex flex-col justify-center cursor-pointer"
                                                            style="width: 14.28%"
                                                            id="{{ $i + 6 }}-{{ $e }}" x-show="edit">
                                                            <a href="{{ route('profile.show', $student->id) }}"
                                                                class="text-sm text-black font-bold">{{ $student->first_name }}
                                                                {{ $student->last_name }}</a>
                                                        </div>
                                ...
                                <div class="w-32 h-10 border cell schedule_cell ui-selected" style="width: 14.28%" id="{{$i+6}}-{{$e}}"></div> --}}
                                            ...
                                            @endif
                                        @else
                                            @if (isset($student) && in_array([$i + 6, $e], $student[1]))
                                                <div onclick="location.href='{{ route('profile.show', $student->id) }}'"
                                                    class="w-32 h-10 border cell schedule_cell flex flex-col justify-center cursor-pointer"
                                                    style="width: 14.28%" id="{{ $i + 6 }}-{{ $e }}"
                                                    x-show="editBtn">
                                                    <a href="{{ route('profile.show', $student->id) }}"
                                                        class="text-sm text-black font-bold">{{ $student->first_name }}
                                                        {{ $student->last_name }}</a>
                                            ...
                                            @else
                                                <div class="w-32 h-10 border cell schedule_cell" style="width: 14.28%"
                                                    id="{{ $i + 6 }}-{{ $e }}"></div>
                                            @endif
                                        @endif
                                    @endfor
                ...
                <div class="flex flex-col space-y-3" x-show="edit" x-transition>
                    <button onclick="toggleCellBlock()" @click="edit = false, editBtn = true"
                        class="bg-red-500 rounded-lg text-white font-bold px-6 py-1 shadow-md h-1/6"
                        wire:click="refresh"><i class="fas fa-times"></i></button>
                    <button @click="showModal1 = true"
                        class="bg-green-500 rounded-lg text-white font-bold px-6 py-1 shadow-md h-5/6"
            ...
            <button onclick="toggleCellBlock()" @click=" edit = true, editBtn = false " wire:click="edit()"
                ...
                x-show="editBtn" x-transition>Edit Schedule</button>
        </div>
        @include('modal')
