<div class="w-full bg-blue pt-8" x-data="{ open: @entangle('popup') }" id="plans" x-cloak>
    ...
    @foreach ($plans as $plan)
      <div class="sm:flex-1 lg:flex-initial lg:w-1/4 rounded-t-lg rounded-tr-none bg-white @if($loop->index == 1) z-10 @else mt-4 @endif flex flex-col shadow-2xl">
        ...
        <div class="text-center mb-8 mt-4">
          <button wire:click="store('{{$plan->n_classes/4}}')" class="inline-block bg-blue-800 text-white px-6 py-4 rounded-lg hover:bg-blue-900 hover:text-white hover:no-underline">Select</button>
        </div>
      </div>
    @endforeach
  ...
  <div x-show="open; setTimeout(() => open = false, 3000)" x-transition.duration.100ms @click.outside="open=false" class="flex justify-center fixed bottom-5 left-5 z-20">
    <div class="w-full px-6 py-3 shadow-2xl flex flex-col items-center border-t sm:w-auto sm:m-4 sm:rounded-lg sm:flex-row sm:border bg-{{$popup_color}}-600 border-{{$popup_color}}-600 text-white">
      <div>
        {{$popup_message}}
      </div>
      <div class="flex mt-2 sm:mt-0 sm:ml-4">
          ...
          <a @click="open = false" class="px-3 py-2 hover:bg-{{$popup_color}}-700 transition ease-in-out duration-300 cursor-pointer"> Dismiss </a>
      </div>
    </div>
  </div>
