<div class="antialiased sans-serif bg-white w-full rounded-xl" x-data="{ showUsers: true, addUser: false, editUser: @entangle('editUser'), deleteUser: false, editUsers: false }">
    <style>
        [type="checkbox"] {
        ...
        <div class="container mx-auto py-6 px-4" x-data="datatables, rowDetail = @entangle('selectedRowsLenght')">
            ...
            @if ($selectedRowsLenght > 0)
                <div class="bg-green-200 fixed top-0 left-0 right-0 z-40 w-full shadow">
                    <div class="container mx-auto px-4 py-4 flex justify-between">
                        ...
                        <div class="flex">
                            <button
                                class="flex items-center justify-center hover:bg-green-300 w-10 h-10 p-1 rounded-full"
                            ...
                            <button
                                class="flex items-center justify-center hover:bg-green-300 w-10 h-10 p-1 rounded-full"
                                wire:click="restore" @click="selectAllCheckbox($event)">
            ...
            <div class="mb-4 flex justify-between items-center">
                ...
                <div class="flex space-x-3">
                    <button class="px-4 py-2 text-white tracking-wider bg-green-500 hover:bg-green-700 rounded"
                        @click="addUser = true">Add User</button>
                    <div class="shadow rounded-lg flex">
                        <div class="relative">
                            <button @click.prevent="open = !open"
                                ...
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 ml-1" width="24"
            ...
            <div class="overflow-x-auto bg-white rounded-lg shadow overflow-y-none relative" style="height: 405px;">
                            ...
                            <tr class="@if ($user->status != 1) text-gray-400 @else text-gray-700 @endif">
                                ...
                                <td class="border-dashed border-t border-gray-200 userUsername">
                                    <a href="{{ route('profile.show', $user->id) }}"
                                        class="px-6 py-3 flex items-center hover:underline hover:text-blue-500">{{ $user->username }}</a>
                                </td>
                                ...
                                <td class="border-dashed border-t border-gray-200 userRole">
                                    <span
                                        class="px-6 py-3 flex items-center">{{ $user->getRoleNames()->first() }}</span>
                                </td>
                                <td class="border-dashed border-t border-gray-200 userStatus">
                                    <span
                                        class="@if ($user->status == 1) text-green-600 @else text-red-600 @endif font-bold px-6 py-3 flex items-center">
                                        @if ($user->status == 1)
                                            Active
                                        @else
                                            Inactive
                                ...
                                <td class="border-dashed border-t border-gray-200 flex space-x-3 px-6 py-2">
                                    <button class="flex items-center" @click="editUser = true"
                                        wire:click="edit({{ $user->id }})">
                                        <i class="fas fa-user-edit"></i>
                                    </button>
                                    <a href="{{ route('impersonate', $user->id) }}" class="flex items-center">
    ...
    <div class="bg-black bg-opacity-50 z-10 fixed top-0 left-0 w-full h-full flex items-center justify-center"
        x-show="addUser" x-transition x-cloak>
            ...
            <div class="max-w-xl m-4 p-10 bg-white rounded shadow-2xl space-y-1" @click.outside="addUser = false">
                ...
                <div class="flex pt-4 justify-end">
                    <button class="px-4 py-1 text-white font-light tracking-wider bg-gray-900 rounded"
                        @click="addUser = false" wire:click="create">Create</button>
                </div>
            </div>
        </div>
        {{-- </div> --}}
        ...
        <div class="leading-loose">
            <div class="max-w-xl m-4 p-10 bg-white rounded shadow-2xl space-y-1" @click.outside="editUser = false">
                ...
                @error('email')
                    <p class="text-red-500 text-sm">{{ $message }}</p>
                @enderror
                <select class="w-full px-2 py-2 text-gray-700 bg-gray-200 rounded" name="user_role" required
                    aria-label="Role" wire:model="role">
                    <option value="1" @if ($role == 1) selected @endif>Guest</option>
                    <option value="2" @if ($role == 2) selected @endif>Student</option>
                    <option value="3" @if ($role == 3) selected @endif>Teacher</option>
                    <option value="4" @if ($role == 4) selected @endif>Admin</option>
                </select>
                @error('role')
                    <p class="text-red-500 text-sm">{{ $message }}</p>
                @enderror
    ...
    {{-- USERS.DELETE --}}
    <div class="bg-black bg-opacity-50 z-10 fixed top-0 left-0 w-full h-full flex items-center justify-center"
        x-show="deleteUser" x-transition x-cloak>
        <div class="leading-loose">
            <div class="max-w-xl m-4 p-10 bg-white rounded shadow-2xl space-y-1" @click.outside="deleteUser = false">
                ...
                <div class="flex mb-8 py-4 w-full border-b text-lg">
                    <p>Are you sure you want to delete @if ($selectedRowsLenght == 1)
                            this user?
                            @endif @if ($selectedRowsLenght > 1)
                                these {{ $selectedRowsLenght }} users?
                            @endif
                    </p>
                </div>
                <div class="flex pt-4 justify-end space-x-2">
                    <button class="px-4 py-1 text-white font-light tracking-wider bg-gray-400 rounded"
                        @click="deleteUser = false">Cancel</button>
                    <button class="px-4 py-1 text-white font-light tracking-wider bg-red-600 rounded"
                        @click="deleteUser = false" wire:click="delete"
                        @click="selectAllCheckbox($event)">Confirm</button>
                </div>
            </div>
        </div>
    </div>
    ...
    {{-- USERS.BULK.EDIT --}}
    <div class="bg-black bg-opacity-50 z-10 fixed top-0 left-0 w-full h-full flex items-center justify-center"
        x-show="editUsers" x-transition x-cloak>
        <div class="leading-loose">
            <div class="max-w-xl m-4 p-10 bg-white rounded shadow-2xl space-y-1" @click.outside="editUsers = false">
                ...
                <div class="flex mb-8 py-4 w-full border-b text-lg">
                    <input class="w-full px-2  py-2 text-gray-700 bg-gray-200 rounded" name="user_role"
                        type="number" required placeholder="Role" min="1" max="4" step="1"
                        aria-label="Role" wire:model="role">
                </div>
                <div class="flex pt-4 justify-end space-x-2">
                    <button class="px-4 py-1 text-white font-light tracking-wider bg-gray-900 rounded"
                        @click="editUsers = false" wire:click="bulkEdit"
                        @click="selectAllCheckbox($event)">Save</button>
                </div>
            </div>
        </div>
    </div>
