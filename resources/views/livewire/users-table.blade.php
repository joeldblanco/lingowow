<div class="antialiased bg-white w-full rounded-xl p-4" x-data="{showUserInfo: @entangle('showUserInfo'), createUser: false}">
    <div class="w-full flex justify-between my-2">
        <div class="text-xl font-bold text-gray-800 my-5 ml-5 flex space-x-2 items-start">
            <button class="px-3 py-2 border border-gray-300 rounded-md font-bold hover:bg-gray-200 hover:border-white hover:text-gray-800 @if($role == 1) bg-gray-400 text-gray-100 @endif" wire:click="selectRole(1)">Guests @if($role == 1) ({{count($models)}}) @endif </button>
            <button class="px-3 py-2 border border-gray-300 rounded-md font-bold hover:bg-gray-200 hover:border-white hover:text-gray-800 @if($role == 2) bg-gray-400 text-gray-100 @endif" wire:click="selectRole(2)">Students @if($role == 2) ({{count($models)}}) @endif </button>
            <button class="px-3 py-2 border border-gray-300 rounded-md font-bold hover:bg-gray-200 hover:border-white hover:text-gray-800 @if($role == 3) bg-gray-400 text-gray-100 @endif" wire:click="selectRole(3)">Teachers @if($role == 3) ({{count($models)}}) @endif </button>
            <button class="px-3 py-2 border border-gray-300 rounded-md font-bold hover:bg-gray-200 hover:border-white hover:text-gray-800 @if($role == 4) bg-gray-400 text-gray-100 @endif" wire:click="selectRole(4)">Admins @if($role == 4) ({{count($models)}}) @endif </button>
        </div>
        <div class="flex flex-col justify-center">
            <button class="border p-3 rounded-md hover:bg-gray-400 hover:text-white" @click="createUser = true">Add User</button>
        </div>
    </div>
    <div class="flex flex-col space-y-8">
        <div class="flex flex-row w-full gap-x-4 gap-y-4 flex-wrap justify-evenly items-stretch">
            @foreach ($models as $user)
                <div class="flex flex-col border space-y-5 border-gray-100 hover:border-blue-500 rounded-xl p-6 bg-gray-50 justify-between" style="width: 32%">
                    <div class="flex justify-between">
                        <div class="w-2/12">
                            <img src="{{Storage::url($user->profile_photo_path)}}" alt="profile_pic" class="rounded-full">
                        </div>
                        <button class="rounded-full w-10 h-10 bg-white active:bg-gray-200 transition duration-100 ease-in-out" wire:click="showInfo({{$user->id}})" {{--@click="showUserInfo = true"--}}>
                    ...
                    <div class="flex flex-col space-y-8 mt-0">
                        ...
                        <p class="text-gray-700 text-sm">Lorem ipsum dolor sit amet consectetur adipisicing elit. Laborum accusantium similique cupiditate doloribus. Fuga, sint. Voluptas quos rerum maiores! Animi cumque hic nesciunt at ut eum amet sequi laboriosam eius!</p>
                    ...
                    <div class="flex justify-between space-x-4">
                        <a href="#" class="border border-blue-500 px-2 py-1 w-1/2 rounded-md text-center text-blue-500 font-bold">Messages</a>
                        <a href="#" class="border border-green-500 px-2 py-1 w-1/2 rounded-md text-center text-green-500 font-bold">Edition</a>
                    </div>
                </div>
        ...
        <div class="relative w-10/12 mx-2 sm:mx-auto mt-10 mb-24 opacity-100">
            ...
            >
                <main class="p-3 text-center py-20">
                    ...
                    <p class="text-2xl text-gray-600 font-bold">{{$selected_teacher->first_name}} {{$selected_teacher->last_name}}</p>
                                                                    ...
                                                                    @if(in_array([$i+6,$e],$students_schedules))
                                                                        @foreach ($students as $student)
                                                                            @if(in_array([$i+6,$e],$student[1]))
                                                                                <div onclick="location.href='{{route('profile.show',$student->id)}}'" class="w-32 h-10 border cell schedule_cell ui-selected flex flex-col justify-center cursor-pointer" style="width: 14.28%" id="{{$i+6}}-{{$e}}">
                                                                                    <p class="text-sm text-black font-bold">{{$student->first_name}} {{$student->last_name}}</p>
                                                                                </div>
                                                                            @endif
                                                                        @endforeach
                                                                    @else
                                        ...
                                        <tr class="flex">
                                            <td class="flex w-10/12 items-start">
                                                <div class="flex space-x-3">
                                                    <img class="rounded-full w-10 h-10" src="{{Storage::url($student->profile_photo_path)}}" alt="profile_picture">
        ...
        {{-- <div class="container mx-auto py-6 px-4"> --}}
            <div class="leading-loose">
                <div class="max-w-xl m-4 p-10 bg-white rounded shadow-2xl space-y-1" @click.outside="createUser = false">
                    ...
                    <select class="w-full px-2 py-2 text-gray-700 bg-gray-200 rounded" name="roles" required wire:model="user_role">
                    ...
                    <div class="flex pt-4 justify-end">
                        <button class="px-4 py-1 text-white font-light tracking-wider bg-gray-900 rounded" @click="createUser = false" wire:click="createUser">Create</button>
                    </div>
                </div>
            </div>
        {{-- </div> --}}
