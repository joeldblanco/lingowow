    ...
    <div class="bg-white font-sans">
                        ...
                        <div>
                            ...
                            <div class="pb-6 space-y-1">
                                ...
                                <input type="number" name="classes" id="classes" placeholder="0" min="0"
                                    step="1" required value="{{ $plan->monthly_classes }}"
                            ...
                            <div class="pb-6 space-y-1">
                                ...
                                <input type="number" name="price" id="price" placeholder="0.00" min="0"
                                    step="0.01" required value="{{ $plan->price }}"
                                ...
                                <select id="product_id" name="product_id"
                                    ...
                                    @foreach ($products as $product)
                                        <option value="{{ $product->id }}"
                                            @if (!empty($plan->product) && $product->id == $plan->product->id) selected @endif>{{ $product->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="pt-6 space-y-1">
                                ...
                                <select id="features-select"
                                    ...
                                    @foreach ($features as $feature)
                                        @if (!$plan->features()->wherePivot('status', 1)->get()->contains($feature->id))
                            ...
                            <div class="pt-6 space-y-1">
                                ...
                                <select id="features-not-included-select"
                                    ...
                                    @foreach ($features as $feature)
                                        @if (!$plan->features()->wherePivot('status', 0)->get()->contains($feature->id))
                                            <option value="{{ $feature->id }}" data-value="{{ $feature->id }}">
                                                {{ $feature->name }}</option>
                                        @endif
                                    @endforeach
        ...
        <script>
            ...
            $(document).ready(function() {
                selectedFeatures = @json($plan->features()->wherePivot('status', 1)->get());
                ...
                selectedFeaturesNotIncluded = @json($plan->features()->wherePivot('status', 0)->get());
